...
import pandas as pd
from file_processing import sample_analyzer, measurement_analyzer, eight_pixel_data_analyzer, eight_pixel_sample_analyzer
...
class PVReportGenerator_2_Pixel:
    
    def __init__(self, file_path, experimenter, metadata):

        self.file_path = file_path
        self.experimenter = experimenter
        self.analyzed = measurement_analyzer(file_path)
        self.timestamp = datetime.now().strftime("%H%M%S_%d%m%Y")
        self.pdf_filename = f"2-Pixel_data_report_{self.timestamp}_{self.experimenter}.pdf"
        self.pdf_path = os.path.join(reports_dir, self.pdf_filename)
    ...
    def _add_sheet_results(self, sheet_name, graph_count, sample_count, next_sheet_name=None):
        ...
        # Create an instance of sample_analyzer for each sheet
        self.processor = sample_analyzer(self.file_path, sheet_name, next_sheet_name=next_sheet_name)
    ...
    def _add_final_page(self, graph_count):
        ...
        plot = measurement_analyzer.plot_boxplot(self.analyzed.pce.fw, self.analyzed.pce.rv, 'PCE values', 'PCE (%)')
        ...
        plot = measurement_analyzer.plot_boxplot(self.analyzed.ff.fw, self.analyzed.ff.rv, 'Fill Factor values', 'FF')
        ...
        plot = measurement_analyzer.plot_boxplot(self.analyzed.jsc.fw, self.analyzed.jsc.rv, 'Short Circuit Current Densities', 'Jsc (mA/cm²)')
        ...
        plot = measurement_analyzer.plot_boxplot(self.analyzed.voc.fw, self.analyzed.voc.rv, 'Open Circuit Voltage values', 'Voc (V)')
...
class PVReportGenerator_8_Pixel:
    
    def __init__(self, jv_file_paths, experimenter):

        self.jv_file_paths = jv_file_paths
        self.experimenter = experimenter
        self.timestamp = datetime.now().strftime("%H%M%S_%d%m%Y")
        self.pdf_filename = f"8-Pixel_data_report_{self.timestamp}_{self.experimenter}.pdf"
        self.pdf_path = os.path.join(reports_dir, self.pdf_filename)
        self.current_page_number = 1  # Initialize page counter
        self.pce_values = []  # Store PCE values for boxplot
        self.c = canvas.Canvas(self.pdf_path, pagesize=A4)
    ...
    def _add_sheet_results(self, sample, px, jv_data_path, performance_data_path, graph_count):
        ...
        # Parse the material and access the metadata of the sample
        parser = eight_pixel_data_analyzer(jv_data_path, performance_data_path, sample, px)
    ...
    def _add_box_plot_page(self, graph_count, filepaths):
        """Generates the final page with boxplots arranged in a 2x2 grid, with the title below the plots."""
        page_width, page_height = A4  # Get the page dimensions
        self.analyzed_8_pixel_sample =  eight_pixel_sample_analyzer(filepaths)
        ...
        # Plotting PCE Values (Top-left)
        plot = measurement_analyzer.plot_boxplot(self.analyzed_8_pixel_sample.pce.fw, self.analyzed_8_pixel_sample.pce.rv, 'PCE values', 'PCE (%)')
        self.c.drawImage(plot, margin_x, margin_y + y_gap, width=plot_width, height=plot_height)
        self.c.setFont('Helvetica', 10)
        self.c.drawString(margin_x, margin_y + y_gap + 185, "(a)")

        # Plotting Fill Factor Values (Top-right)
        plot = measurement_analyzer.plot_boxplot(self.analyzed_8_pixel_sample.ff.fw, self.analyzed_8_pixel_sample.ff.rv, 'Fill Factor values', 'FF')
        self.c.drawImage(plot, margin_x + x_gap, margin_y + y_gap, width=plot_width, height=plot_height)
        self.c.drawString(margin_x + x_gap , margin_y + y_gap + 185, "(b)")

        # Plotting J sc (Bottom-left)
        plot = measurement_analyzer.plot_boxplot(self.analyzed_8_pixel_sample.isc.fw, self.analyzed_8_pixel_sample.isc.rv, 'Short Circuit Current Densities', 'Isc (mA)')
        self.c.drawImage(plot, margin_x, margin_y, width=plot_width, height=plot_height)
        self.c.drawString(margin_x, margin_y + 185, "(c)")

        # Plotting V oc (Bottom-right)
        plot = measurement_analyzer.plot_boxplot(self.analyzed_8_pixel_sample.voc.fw, self.analyzed_8_pixel_sample.voc.rv, 'Open Circuit Voltage values', 'Voc (V)')
        self.c.drawImage(plot, margin_x + x_gap, margin_y, width=plot_width, height=plot_height)
        self.c.drawString(margin_x + x_gap, margin_y + 185, "(d)")
    ...
    def save_pdf(self):
        ...
        for sample, pixels in self.jv_file_paths.items():
            odd_index_filepaths = [filepaths[i] for filepaths in pixels.values() for i in range(1, len(filepaths), 2)] # To store performance data

            for px, files in pixels.items():
                for j in range (0, len (files),2):
                    jv_data_path = files[j]
                    performance_data_path = files [j+1]
                    self._add_sheet_results(sample, px, jv_data_path, performance_data_path, graph_count)

            self._add_box_plot_page(graph_count, odd_index_filepaths)
